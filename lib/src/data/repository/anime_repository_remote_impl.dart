...
import '../../utils/error/runtime_exception.dart';
...
class AnimeRepositoryRemoteImpl extends BaseRepositoryRemote implements AnimeRepositoryRemote {
  ...
  Future<DataStatePagination<List<AnimeData>, PaginationData>> getAnimeSearch() async {
    ...
    if (dataStatePaginationAnimeResponse is Error) {
      return DataStatePaginationError(
          dataStatePaginationAnimeResponse.exception!
      );
    } else if (dataStatePaginationAnimeResponse is DataStatePaginationSuccess) {
      return DataStatePaginationSuccess(
        dataStatePaginationAnimeResponse.data?.dataAnimeResponses!.map((dataAnimeResponse) => _dataAnimeResponseToAnimeData(dataAnimeResponse)).toList() ?? <AnimeData>[]
      );
    } else {
      return DataStatePaginationError(
  ...
  AnimeData _dataAnimeResponseToAnimeData(DataAnimeResponse dataAnimeResponse) =>
      AnimeData(
        ...
        dataAnimeResponse.type != null ? TypeAnimeConverter().encode(dataAnimeResponse.type!) : null,
        ...
        dataAnimeResponse.producers?.map((otherItemResponse) => _otherItemResponseToStudioData(otherItemResponse)).toList(),
        dataAnimeResponse.licensors?.map((otherItemResponse) => _otherItemResponseToStudioData(otherItemResponse)).toList(),
        dataAnimeResponse.studios?.map((otherItemResponse) => _otherItemResponseToStudioData(otherItemResponse)).toList(),
        <GenreData>[
          ...?dataAnimeResponse.genres?.map((otherItemResponse) => _otherItemResponseToGenreData(otherItemResponse)).toList(),
          ...?dataAnimeResponse.themes?.map((otherItemResponse) => _otherItemResponseToGenreData(otherItemResponse)).toList(),
          ...?dataAnimeResponse.demographics?.map((otherItemResponse) => _otherItemResponseToGenreData(otherItemResponse)).toList()
        ]
      );
  
  StudioData _otherItemResponseToStudioData(OtherItemResponse otherItemResponse) => StudioData(otherItemResponse.malId, otherItemResponse.type, otherItemResponse.name, otherItemResponse.url);

  GenreData _otherItemResponseToGenreData(OtherItemResponse otherItemResponse) => GenreData(otherItemResponse.malId, otherItemResponse.type, otherItemResponse.name, otherItemResponse.url);
}
