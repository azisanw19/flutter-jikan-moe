...
import 'package:anime_list/src/utils/error/repository_local_exception.dart';
...
class AnimeRepositoryLocalImpl extends BaseRepositoryLocal
  ...
  @override
  Future<DataState<List<AnimeData>>> getListAnimeSeasonNow(int limit,
      int offset) async {
  ...
  @override
  Future<DataState<AnimeData>> getDetailAnime(int malIdAnime) async {
    DataState<AnimeTable> dataStateAnime = await _getAnimeFromId(malIdAnime);

    if (dataStateAnime is DataStateError)
      return DataStateError(dataStateAnime.exception!);

    if (dataStateAnime  is DataStateSuccess) {
      return DataStateSuccess(dataStateAnime.data!.toAnimeData());
    }

    return DataStateError(RepositoryLocalException("Something wrong"));
  }
  ...
  Future<DataState<void>> _insertAnimeSeasonNowTransaction(
      ...
      getStateOf(
          request: () =>
              _animeDao.insertAnimeAndSeasonNowTransaction(
                  listAnimeEntity,
                  listStudioEntity,
                  listGenreEntity,
                  listRelationTitleSynonymAndAnime,
  ...
  Future<DataState<List<AnimeTable>>> _getAnimeTableSeasonNow(int limit,
      int offset) =>
      getStateOf(
  ...
  Future<DataState<AnimeTable>> _getAnimeFromId(int malIdAnime) =>
      getStateOf(request: () => _animeDao.getAnimeTableFromId(malIdAnime));
}
